# Migration Guide — HOLON_AGENT_* Environment Variable Rename
...
All agent-specific and vendor-specific environment variables have been unified under a
single `HOLON_AGENT_*` namespace. This is a **hard cutover** — legacy variable names are
no longer honoured.
...
The most important change: a single `HOLON_AGENT_KEY` replaces all provider-specific
API key variables. `sandbox-executor` injects this key into each agent container and
the appropriate internal translation (`_apply_generic_token`) maps it to the agent CLI's
native credential variable at runtime.
